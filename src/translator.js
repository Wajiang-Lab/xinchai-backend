...
async function translateItems(items, apiKey) {
  ...
  for (let i = 0; i < items.length; i += batchSize) {
    ...
    const systemPrompt = '你是一个翻译助手。将以下英文技术新闻的「标题」和「摘要」翻译成中文。保持技术术语准确，保留专业名词不翻译（如 GitHub、Reddit、API、RSS、LLM、Agent 等）。按相同序号输出。';

    try {
      const translated = await callDoubao(systemPrompt, prompts, apiKey);
      if (translated) {
      ...
      console.log(`  ✅ 豆包翻译: 第 ${i + 1}-${Math.min(i + batchSize, items.length)} 条`);
    } catch (err) {
      console.warn(`  ⚠️ 豆包翻译失败: ${err.message}`);
    }
...
function callDoubao(systemPrompt, userContent, apiKey) {
      ...
      res.on('end', () => {
        if (res.statusCode === 200) {
          try {
            ...
            reject(new Error('解析豆包响应失败'));
          }
        } else {
          reject(new Error(`豆包 API ${res.statusCode}: ${data.slice(0, 100)}`));
        }
      });
    });
    req.on('error', reject);
    req.on('timeout', () => { req.destroy(new Error('豆包 API 超时')); });
    req.write(body);
    req.end();
  });
}
